...
export async function POST(req: Request) {
  try {
    ...
    // reply-to: handig om direct op de klant te kunnen antwoorden
    ...
    // ✅ Resend gebruikt reply_to (snake_case) 
    const result = await resend.emails.send({
      from: fromEmail,           // bv: "Bas Foto <onboarding@resend.dev>"
      to: [toEmail],             // array is safest
      reply_to: replyTo || undefined,
      subject,
      html,
    });

    // Als Resend error teruggeeft: toon hem aan de client (zodat jij ‘m ziet in Network)
