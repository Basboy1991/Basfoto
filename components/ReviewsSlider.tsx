...
export default function ReviewsSlider({ reviews }: { reviews: Review[] }) {
  ...
  }, [current?.quote, current?.name, current?.location, current?.stars]);

  const stars = useMemo(() => {
    const s = current?.stars ?? 0;
    if (!s) return null;
  ...
  return (
    <section className="mt-20">
      ...
      <div
        className="relative mx-auto max-w-3xl overflow-hidden rounded-3xl bg-[var(--accent-soft)] px-8 py-12 text-center"
        style={{
      ...
      >
        ...
        >
          {/* ⭐ Sterren – groter & premium */}
          {stars && (
            <div className="mb-6 flex justify-center gap-1 text-3xl md:text-4xl text-[#D4AF37]">
              {stars}
            </div>
          )}
